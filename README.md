...
- Создать интерфейс Supplier<P> с методом P get(), возвращающим новый пустой контейнер.
- Создать интерфейс BiConsumer<P, T> с методом void accept(P collection, T item), выполняющим добавление элемента.
- Реализовать метод collect, который обходит исходный список и собирает данные в контейнер.
### Код
...
### Пример вывода программы
```
Результат: {2=[zx, qw], 5=[asdfg], 6=[qwerty]}
```
### Пример ввода
``` 
Введите 3 целых числа:
